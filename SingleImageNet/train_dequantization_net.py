...
from SingleImageNet import util, dequantization_net
from DataProcess import dataset
from Dual_SingleImageNet import quantization_net

# --- graph
class train_dequantization_net(object):
    def __init__(self, sess, args):
        ...
        self.it_num = args.deq_epoch
        self.logdir_path = args.deq_indi_log_dir
        self.hdr_prefix = args.hdr_prefix

        self.dual_lamada = 0.1
    ...
    def build_graph(
        ...
        with tf.variable_scope("Dequantization_Net"):
            self.model = dequantization_net.Dequantization_net(is_train=is_training)
            self.pred_ldr = self._clip(self.model.inference(self.jpeg_img_float))

        """ build dual model """
        with tf.variable_scope("Quantization_Net"):
            self.dual_model = quantization_net.Quantization_net(is_train=is_training)
            self.dual_pred_jpg = self.dual_model.get_output(self.pred_ldr)

        """ compute dual loss """
        self.dual_final_loss = util.get_l2_loss_with_mask(self.dual_pred_jpg, self.jpeg_img_float)

        """ compute primary loss """
        self.loss = util.get_l2_loss_with_mask(self.pred_ldr, self.ldr)
        ...
        """ comput some else """
        mse = tf.reduce_mean((self.pred - self.ldr) ** 2)
