...
class Test_Eye(object):
    def __init__(self, sess, args):
        ...
        self.test_imgs = 'result/HDR-Real/LDR/'
        self.hal_train_ckpt = '/home/shelei/HDR/Test_ckpt/dBD_iter3_hal_synth_100k_d15/best/model.ckpt'
        self.output_path = 'result/HDR-Real/dBD_iter3_hal_synth_100k_d15'

        self._clip = lambda x: tf.clip_by_value(x, 0, 1)
        self.use_iter_hal = False
    ...
    def build_graph(
        ...
        with tf.variable_scope("Hallucination_Net"):
            net_test, vgg16_conv_layers_test = hallucination_net.model(pred_clipped_hdr, self.batch_size, False)
            y_predict_test = net_test.outputs
            y_predict_test = tf.nn.relu(y_predict_test)
            self.pred_hdr = (pred_clipped_hdr) + alpha * y_predict_test

        if self.use_iter_hal:
            with tf.variable_scope("Clipping_Net"):
                self.cl_net = Clipping_Net()
                self.dual_pred_clipped_hdr, self.dual_pred_base, self.dual_pred_detail = self.cl_net.model(self.pred_hdr)

            for i in range(3):
                with tf.variable_scope("Hallucination_Net", reuse=True):
                    net_test, vgg16_conv_layers_test = hallucination_net.model(self.dual_pred_clipped_hdr, self.batch_size, False)
                    y_predict_test = net_test.outputs
                    y_predict_test = tf.nn.relu(y_predict_test)
                    self.pred_hdr = (self.dual_pred_clipped_hdr) + alpha * y_predict_test
    ...
    def run_test(self):
        ...
        for ldr_img_path in ldr_imgs:
            ...
            HDR_out_val = self.sess.run(self.pred_hdr, {
                ldr: [ldr_val],
                is_training: False,
            })
