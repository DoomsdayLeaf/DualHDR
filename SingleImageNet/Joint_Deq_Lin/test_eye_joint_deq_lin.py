...
class Test_Eye(object):
    def __init__(self, sess, args):
        ...
        self.test_imgs = args.test_eye_data
        self.joint_train_ckpt = 'ckpt/dual_deq_lin_100_d1_d1/best/model.ckpt'
        self.output_path = 'result/HDR-Eye/ddeq_d10_deq_share_lin_d0_0_1_dBD_iter3_d10/'

        self.best_deq_ckpt = "ckpt/dual_deq_synth_L2_d100k_d10/best/model.ckpt"
        self.best_lin_ckpt = 'ckpt/deq_share_lin_clipped_synth_100k_d0_0_1/best/model.ckpt'
        self.best_hal_ckpt = "ckpt/dBD_iter3_hal_synth_100k_d10/best/model.ckpt"
        self.use_best_ckpt = True
    ...
    def run_test(self):
        ...
        if self.use_best_ckpt:
            restorer0 = tf.train.Saver(
                var_list=[var for var in tf.get_collection(tf.GraphKeys.VARIABLES) if 'Dequantization_Net' in var.name])
            restorer0.restore(self.sess, self.best_deq_ckpt)

            restorer2 = tf.train.Saver(var_list=[var for var in tf.get_collection(tf.GraphKeys.VARIABLES) if
                                                 'crf_feature_net' in var.name or 'ae_invcrf_' in var.name])
            restorer2.restore(self.sess, self.best_lin_ckpt)

            restorer3 = tf.train.Saver(
                var_list=[var for var in tf.get_collection(tf.GraphKeys.VARIABLES) if 'Hallucination_Net' in var.name])
            restorer3.restore(self.sess, self.best_hal_ckpt)
        else:
            restorer_all = tf.train.Saver(var_list=[var for var in tf.get_collection(tf.GraphKeys.VARIABLES) if
                                                 'Dequantization_Net' in var.name or 'crf_feature_net' in var.name or 'ae_invcrf_' in var.name])
            restorer_all.restore(self.sess, self.joint_train_ckpt)
