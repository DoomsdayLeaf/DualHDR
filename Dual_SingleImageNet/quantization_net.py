...
class Quantization_net(object):
    def __init__(self, batch_size=8, is_train=True):
    ...
    def get_output(self, input_images):
        quantized_hdr = tf.round(input_images * 255.0)
        quantized_hdr_8bit = tf.cast(quantized_hdr, tf.uint8)
        jpeg_img_list = []
        for i in range(quantized_hdr_8bit.shape[0]):
