from SingleImageNet.net import BaseNet, AggNet
import tensorflow.contrib.slim as slim
import tensorflow as tf
import numpy as np
import os
from SingleImageNet.util import get_tensor_shape
...
class CrfFeatureNet(BaseNet):
    ...
    def _get_output(
        ...
        if self.share_params:
            scope_first = "crf_feature_net"
        else:
            scope_first = self.scope

        with tf.variable_scope(scope_first, reuse=self.share_params):
    ...
    def overwrite_init(self, sess):
        ...
        np_var = {}
        refine_np_var(
            np.load('SingleImageNet/crf_net_v2.npy', encoding='latin1').item(),
            np_var,
        )
...
class Non_Linearization_net(AggNet):

    def __init__(self, share_params=False):
        self.crf_feature_net = CrfFeatureNet(share_params=share_params)
        self.decode_spec = []
        self.s = 1024
        self.n_p = 12
        self.act = tf.nn.tanh
    ...
    def parse_dorf(self):
        ...
        with open(os.path.join('SingleImageNet/dorfCurves.txt'), 'r') as f:
            lines = f.readlines()
    ...
    def parse_emor(self):
        ...
        with open(os.path.join('SingleImageNet/emor.txt'), 'r') as f:
            lines = f.readlines()
            lines = [line.strip() for line in lines]

        e = self.parse(lines, 'E =')
        f0 = self.parse(lines, 'f0 =')
        h = np.stack([self.parse(lines, 'h(%d)=' % (i + 1)) for i in range(11)], axis=-1)
    ...
    def crf_pca_w_2_crf(
        ...
        _, F0, H = self.parse_emor()
        b, _, = get_tensor_shape(crf_pca_w)

        crf_pca_w = tf.expand_dims(crf_pca_w, -1)  # [b, 11, 1]

        F0 = tf.constant(F0)  # [   s   ]
        F0 = tf.reshape(F0, [1, -1, 1])  # [1, s, 1]

        H = tf.constant(H)  # [   s, 11]
        H = tf.expand_dims(H, 0)  # [1, s, 11]
        H = tf.tile(H, [b, 1, 1])  # [b, s, 11]
    ...
    def _get_output(self, img, is_training):
        ...
        crf = self.decode(feature)
        # [b, 1024]

        crf = self._increase(crf)
        # [b, 1024]

        crf = tf.cast(crf, tf.float32)
        # float32
